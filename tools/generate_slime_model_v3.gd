...
const MODEL_PATH := "res://assets/models/slime_hero_v3.res"
const LAT_STEPS := 48
const SIDE_STEPS := 96
const EYE_STEPS := 32
const EYE_RINGS := 4
...
func _generate() -> void:
	...
	model.resource_name = "Slime Hero v3 — layered gel and coherent motion"
...
func _body_point(phi: float, theta: float) -> Vector3:
	...
	var ring := pow(absf(cos(phi)), 0.62)
	var height := 0.395 * _signed_power(vertical, 0.62)
	var lower_fullness := 1.0 + 0.10 * maxf(0.0, -vertical) - 0.09 * maxf(0.0, vertical)
	var contour := 1.0 + 0.035 * sin(theta * 3.0 + 0.6) + 0.022 * sin(theta * 5.0 - phi * 1.3)
	var x := 0.47 * ring * _signed_power(cos(theta), 0.69) * lower_fullness * contour
	var z := 0.47 * ring * _signed_power(sin(theta), 0.69) * lower_fullness * contour
	height += 0.028 * sin(theta * 3.0 + 0.4) * ring * maxf(0.0, vertical)
	height -= 0.018 * sin(theta * 4.0 + 0.7) * ring * maxf(0.0, -vertical)
...
func _front_surface_z(x: float, y: float) -> float:
	var vertical := pow(clampf(absf(y) / 0.395, 0.0, 1.0), 1.0 / 0.62) * signf(y)
	var ring := pow(maxf(0.0, 1.0 - vertical * vertical), 0.62 * 0.5)
	var fullness := 1.0 + 0.10 * maxf(0.0, -vertical) - 0.09 * maxf(0.0, vertical)
	var horizontal_radius := maxf(0.001, 0.47 * ring * fullness)
	var horizontal := pow(clampf(absf(x) / horizontal_radius, 0.0, 1.0), 2.0 / 0.69)
	return -horizontal_radius * pow(maxf(0.0, 1.0 - horizontal), 0.69 * 0.5) * 1.04
