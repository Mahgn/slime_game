...
func _ready() -> void:
	...
	_model_instance.name = "SlimeHeroModelV3"
...
func _process(delta: float) -> void:
	...
	_model_instance.rotation.z = lerpf(_model_instance.rotation.z, move_ratio * stride * 0.19 + _action_pulse * 0.075, blend)
	...
	var local_velocity := player.visual_root.global_basis.inverse() * player.velocity
	var target_motion := Vector2(-local_velocity.x, -local_velocity.z) / SlimeController.MOVE_SPEED
	_motion_lag = _motion_lag.lerp(target_motion.limit_length(1.0), minf(1.0, delta * 6.0))
	var gel_energy := 0.15 + move_ratio * 1.25 + _landing_pulse * 1.1 + _action_pulse * 0.85
	for material in _animated_materials:
		...
		material.set_shader_parameter("gel_motion", _motion_lag)
